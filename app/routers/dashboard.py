...
def summary(db: Session = Depends(get_db),
    ...
    income = db.query(func.sum(Transaction.amount))\
               .filter(Transaction.type == TypeEnum.income).scalar() or 0
    expense = db.query(func.sum(Transaction.amount))\
                .filter(Transaction.type == TypeEnum.expense).scalar() or 0
    return {
        "total_income": income,
...
def monthly_trends(db: Session = Depends(get_db),
    ...
    return [{"year": r[0], "month": r[1], "type": r[2], "total": r[3]} for r in results]